...
operacion = ""  # Almacena la operación a realizar
resultado = 0  # Almacena el resultado de la operación

# ------------- PANTALLA ---------------------------
numeroPantalla = StringVar()
numeroPantalla.set("0")  # Número inicial en pantalla
...
def numeroPulsado(num):
	...
	if current == "0":
		if num == "0":  # No concatenar varios 0 a la izquierda
			return
		else:
			numeroPantalla.set(num)  # Quitar el 0 y poner el nuevo número
			return
	numeroPantalla.set(numeroPantalla.get() + num)

# ------------ FUNCION SUMA -----------------------
def suma(num):
	global operacion  # Necesario para cambiar una variable global desde una función
	global resultado
	operacion = "suma"
	resultado += int(num)
	numeroPantalla.set(resultado)

# ------------ FUNCION elResultado (al pulsar igual) ----------------
def elResultado():
	global resultado
...
botonIgual = Button(miFrame, text="=", width=5, command=lambda:elResultado())
